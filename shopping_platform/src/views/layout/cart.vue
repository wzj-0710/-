  ...
  <div class="cart-box" v-if="isLogin && cartList.length > 0">
    ...
    <div class="cart-list">
      <div class="cart-item" v-for="item in cartList" :key="item.goods_id">
        <!--单向绑定，用:value,不能用v-model双向绑定，因为双向绑定会直接使得修改vuex里面的isChecked值，这里只是渲染-->
    ...
    <div class="footer-fixed">
      ...
      <div class="all-total">
        ...
        <div @click = "goPay" v-if="!isEdit" :class="{disabled: selCount === 0}" class="goPay">结算({{ selCount }})</div>
...
export default {
  ...
  // 必须登录过的用户，才能进入对应购物车列表
  created () {
    if (this.isLogin) { this.$store.dispatch('cart/getCartAction') }
  },
  computed: {
